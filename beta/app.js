...
const I18N = {
  ru: {
    ...
    "market.filter.selectedN": "Выбрано: {n}",
    "market.list.title": "Выставить на продажу",
    "market.list.text": "Укажите цену в Nectar. Подарок исчезнет из вашего профиля и появится в маркете.",
    "market.list.invalid": "Цена должна быть целым числом больше нуля.",
    "market.card.changePrice": "Цена",
    "market.card.unlist": "Снять",
    "market.profile.myListings": "Мои предложения",
    "market.profile.noListings": "У вас пока нет активных предложений.",
    "market.changePrice.title": "Изменить цену",
    "market.changePrice.text": "Новая цена в Nectar:",
    "market.unlist.title": "Снять с продажи",
    "market.unlist.text": "Подарок вернётся в ваш профиль. Продолжить?",
    "market.unlist.confirm": "Снять",
    "market.listing.contact": "Связаться с продавцом",
    "market.listing.contactText": "Открыть чат с @{username}?",
    "market.listing.contactAction": "Открыть чат",
    "market.listing.listedAt": "Дата выставления",
    "market.listing.quantity": "Выпущено",
    "market.cart.empty": "Корзина пуста.",
    ...
    "gifts.action.toMarket": "Выставить на продажу",
  ...
  en: {
    ...
    "market.filter.selectedN": "{n} selected",
    "market.list.title": "List for sale",
    "market.list.text": "Enter a price in Nectar. The gift will disappear from your profile and appear on the market.",
    "market.list.invalid": "Price must be a positive integer.",
    "market.card.changePrice": "Price",
    "market.card.unlist": "Unlist",
    "market.profile.myListings": "My listings",
    "market.profile.noListings": "You have no active listings yet.",
    "market.changePrice.title": "Change price",
    "market.changePrice.text": "New price in Nectar:",
    "market.unlist.title": "Unlist",
    "market.unlist.text": "The gift will return to your profile. Continue?",
    "market.unlist.confirm": "Unlist",
    "market.listing.contact": "Contact seller",
    "market.listing.contactText": "Open chat with @{username}?",
    "market.listing.contactAction": "Open chat",
    "market.listing.listedAt": "Listed on",
    "market.listing.quantity": "Total issued",
    ...
    "gifts.action.toMarket": "List for sale",
...
async function openUserProfileDialog(userOverride) {
  ...
  // Себе — все свои подарки (минус те, что на маркете). Другому — только in_profile.
  ...
  if (isMe) {
    try {
      const { count: lc } = await supabase.from("market_listings")
        .select("id", { count: "exact", head: true })
        .eq("seller_id", user.id).eq("status", "active");
      listedCount = lc || 0;
    } catch (e) { /* silent */ }
  }
  document.getElementById("user-profile-gifts-count").textContent =
    String(Math.max(0, (totalGifts || 0) - listedCount));
...
const marketState = {
  ...
  loading: false,
  offset: 0,
  filtersApplied: emptyMarketFilters(),
  filtersDraft:   emptyMarketFilters(),
};

function emptyMarketFilters() {
  return { giftId: null, modelIds: [], backgroundNames: [], patternIds: [], sort: "new-desc" };
}

// Цвет каждого background_name — Map<name, "c1|c2">.
// Заполняется один раз ленивым RPC get_background_examples().
let marketBackgroundColors = null;
...
async function ensureMarketBackgroundColors() {
  if (marketBackgroundColors) return;
  marketBackgroundColors = new Map();
  try {
    const { data, error } = await supabase.rpc("get_background_examples");
    if (error) return;
    (data || []).forEach((r) => {
      if (r.background_name && r.background && !marketBackgroundColors.has(r.background_name)) {
        marketBackgroundColors.set(r.background_name, r.background);
      }
    });
  } catch (e) { /* silent */ }
}
...
function closeMarket() {
  ...
  marketState.filtersApplied = emptyMarketFilters();
  marketState.filtersDraft   = emptyMarketFilters();
}
...
// ======================================================
// Маркет: вкладка «Профиль» — пока только «Мои предложения»
// ======================================================

async function renderMarketProfile() {
  const content = document.getElementById("market-content");
  ...
  content.innerHTML = `
    <div class="market-profile">
      <div class="market-profile-header">
        <div class="avatar avatar-large" id="market-profile-avatar">?</div>
  ...
  paintAvatar(document.getElementById("market-profile-avatar"), myProfile);
  document.getElementById("market-profile-name").innerHTML =
...
async function loadMarketMyListings() {
  const el = document.getElementById("market-my-listings");
  if (!el) return;
  ...
  try {
    ...
    const rows = data || [];
    if (!rows.length) {
      el.innerHTML = `<div class="empty">${escapeHtml(t("market.profile.noListings"))}</div>`;
      return;
    }

    if (!giftCatalogCache.length) await loadGiftCatalog();

    el.innerHTML = rows.map(renderMyListingCard).filter(Boolean).join("");
    fillGiftPatternsIn(el);
    bindMyListingCards(rows);
  } catch (e) {
    console.error("loadMarketMyListings:", e);
  }
}
  ...
  return `
    <div class="market-card" data-listing-id="${listing.id}">
      ...
      <div class="market-card-actions">
        <button type="button" class="market-card-buy" data-action="change-price" data-listing-id="${listing.id}">${escapeHtml(t("market.card.changePrice"))}</button>
        <button type="button" class="market-card-cart" data-action="unlist" data-listing-id="${listing.id}">${escapeHtml(t("market.card.unlist"))}</button>
      </div>
    </div>`;
}

function bindMyListingCards(rows) {
  const el = document.getElementById("market-my-listings");
  if (!el) return;
  ...
  el.querySelectorAll(".market-card").forEach((card) => {
    ...
    if (changeBtn) changeBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      marketChangePrice(listingId, listing.price);
    });

    const unlistBtn = card.querySelector('[data-action="unlist"]');
    if (unlistBtn) unlistBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      marketUnlist(listing.user_gift_id);
    });
  });
}

async function marketChangePrice(listingId, currentPrice) {
  ...
  if (!price || price <= 0) {
    await showAlertDialog(t("gifts.error"), t("market.list.invalid"));
    return;
  }
  const { error } = await supabase.from("market_listings")
    .update({ price }).eq("id", listingId);
  if (error) { await showAlertDialog(t("gifts.error"), error.message); return; }
  await loadMarketMyListings();
}

async function marketUnlist(userGiftId) {
  const ok = await showConfirmDialog(
    t("market.unlist.title"),
    t("market.unlist.text"),
    t("market.unlist.confirm")
  );
  if (!ok) return;
...
async function loadMarketListings(reset) {
  ...
  try {
    ...
    if (f.modelIds.length)        q = q.in("user_gifts.model_id", f.modelIds);
    if (f.backgroundNames.length) q = q.in("user_gifts.background_name", f.backgroundNames);
    if (f.patternIds.length)      q = q.in("user_gifts.pattern_id", f.patternIds);
...
async function openMarketListingDetail(listingId, fallbackTx, onClose) {
  ...
  const bodyEl = document.getElementById("market-listing-body");
  const buyBtn = document.getElementById("market-listing-buy");
  const cartBtn = document.getElementById("market-listing-cart");
  ...
  const sellerName = seller ? (seller.display_name || seller.username) : "—";
  ...
  bodyEl.innerHTML = `
    <div class="gift-hero" style="${bg}">
      <div class="gift-hero-pattern" style="display:none;"></div>
      <div class="gift-hero-emoji">${renderGiftModel(displayImg, 140)}</div>
    </div>

    <div class="gift-detail-sub">${escapeHtml(cat.collection || "—")} · ${giftRarityLabel(cat.rarity)}</div>

    <div class="market-listing-seller" id="market-listing-seller">
      <div class="avatar" id="market-listing-seller-avatar">?</div>
      <div class="market-listing-seller-info">
        <div class="market-listing-seller-name">${escapeHtml(sellerName)}</div>
        <div class="market-listing-seller-username">@${escapeHtml(sellerUsername)}</div>
      </div>
      <div class="market-listing-seller-cta">${escapeHtml(t("market.listing.contact"))}</div>
    </div>
    ...
    <div class="gift-info-table">
      ...
      ${bgRow}
      <div class="gift-info-row">
        <span class="gir-label">${escapeHtml(t("market.listing.quantity"))}</span>
        <span class="gir-value">${escapeHtml(qtyValue)}</span>
      </div>
    </div>`;

  paintAvatar(document.getElementById("market-listing-seller-avatar"), seller || { display_name: "?" });

  // Паттерн рисуем асинхронно.
  if (patternIcon) {
    buildPatternMaskUrl(patternIcon).then((bgUrl) => {
      if (!bgUrl) return;
      const el = bodyEl.querySelector(".gift-hero-pattern");
      if (!el) return;
...
async function marketContactSeller(seller) {
  const ok = await showConfirmDialog(
    t("market.listing.contact"),
    tFmt("market.listing.contactText", { username: seller.username || "" }),
    t("market.listing.contactAction")
  );
  if (!ok) return;
  const overlay = document.getElementById("market-listing-overlay");
  if (overlay) overlay.classList.add("hidden");
  currentMarketListingId = null;
  closeMarket();
  try { await openChatWith(seller); } catch (e) { /* silent */ }
}
...
async function renderMarketCart() {
  const content = document.getElementById("market-content");
  if (!content) return;
  content.innerHTML = '<div class="empty">' + escapeHtml(t("empty.loading")) + '</div>';
  ...
  try {
    ...
    const rows = (data || []).filter((r) => r.market_listings && r.market_listings.status === "active");

    // Синхронизируем локальный Set.
    marketCartIds = new Set(rows.map((r) => r.listing_id));
    updateMarketCartBadge();

    if (!rows.length) {
      content.innerHTML = `<div class="empty">${escapeHtml(t("market.cart.empty"))}</div>`;
      return;
    }

    if (!giftCatalogCache.length) await loadGiftCatalog();

    const total = rows.reduce((s, r) => s + (r.market_listings.price || 0), 0);

    let html = `<div class="market-cart-layout">
      <div class="market-cart-list">
        <div class="market-grid">
          ${rows.map((r) => renderMarketCartCard(r.market_listings)).filter(Boolean).join("")}
        </div>
      </div>
      <aside class="market-cart-aside">
        <div class="market-cart-aside-title">${escapeHtml(t("market.cart.total"))}</div>
        <div class="market-cart-aside-sum">${NECTAR_HTML} <b>${total}</b></div>
        <button class="market-cart-buy-all" id="market-cart-buy-all">${escapeHtml(t("market.cart.buyAll"))}</button>
      </aside>
    </div>`;
    ...
    content.querySelectorAll(".market-cart-card").forEach((card) => {
      ...
      if (removeBtn) removeBtn.addEventListener("click", async (e) => {
        e.stopPropagation();
        await marketCartToggle(listingId);
        // Убираем карточку мгновенно.
        card.remove();
        // Если корзина опустела — перерисуем пустое состояние.
        const left = content.querySelectorAll(".market-cart-card").length;
        if (left === 0) renderMarketCart();
        else updateCartAsideTotal();
      });

      const buyBtn = card.querySelector('[data-action="cart-buy"]');
      if (buyBtn) buyBtn.addEventListener("click", async (e) => {
        e.stopPropagation();
        const listing = rows.find((r) => r.listing_id === listingId);
        if (!listing) return;
        const l = listing.market_listings;
        const ok = await showConfirmDialog(
          t("market.buy.title"),
          tFmt("market.buy.text", { price: `${NECTAR_HTML} <b>${l.price}</b>` }),
          t("market.buy.confirm"),
          { html: true }
        );
        if (!ok) return;
    ...
    const buyAllBtn = document.getElementById("market-cart-buy-all");
    if (buyAllBtn) buyAllBtn.addEventListener("click", async () => {
      const ok = await showConfirmDialog(
        t("market.cart.buyAll"),
        tFmt("market.cart.buyAllText", { sum: `${NECTAR_HTML} <b>${total}</b>` }),
        t("market.cart.buyAll"),
        { html: true }
      );
      if (!ok) return;
    ...
    console.error("renderMarketCart:", e);
  }
}

function updateCartAsideTotal() {
  const content = document.getElementById("market-content");
  if (!content) return;
  const cards = content.querySelectorAll(".market-cart-card");
  let sum = 0;
  cards.forEach((c) => { sum += parseInt(c.dataset.price || "0", 10) || 0; });
  const el = content.querySelector(".market-cart-aside-sum");
  if (el) el.innerHTML = `${NECTAR_HTML} <b>${sum}</b>`;
}

function renderMarketCartCard(listing) {
  const ug = listing.user_gifts || {};
  const cat = giftCatalogCache.find((c) => c.id === ug.gift_id);
  if (!cat) return "";
  ...
  return `
    <div class="market-card market-cart-card" data-listing-id="${listing.id}" data-price="${listing.price}">
      ...
      <div class="market-card-actions">
        <button type="button" class="market-card-buy" data-action="cart-buy">${escapeHtml(t("market.card.buy"))}</button>
        <button type="button" class="market-card-cart in-cart" data-action="cart-remove">${escapeHtml(t("market.card.removeFromCart"))}</button>
      </div>
    </div>`;
}

// ======================================================
// Маркет: выставление подарка на продажу
// ======================================================
...
async function marketListGiftDialog(userGiftId) {
  ...
  const { error } = await supabase.rpc("market_list_gift", {
...
function renderMarketFilters() {
  ...
  // Модель — мультивыбор (только если выбран подарок с моделями).
  if (hasModels) {
    chips.push(multiFilterChip("model", f.modelIds, cat.models.map((m) => ({ id: m.id, name: m.name || m.id })), t("market.filter.model")));
  }

  // Фон — мультивыбор.
  chips.push(multiFilterChip("background", f.backgroundNames, null, t("market.filter.background")));

  // Паттерн — мультивыбор.
  chips.push(multiFilterChip("pattern", f.patternIds, null, t("market.filter.pattern")));
...
function multiFilterChip(filterName, ids, allItems, baseLabel) {
  let label = baseLabel;
  if (ids.length === 1) {
    if (allItems) {
      const item = allItems.find((x) => x.id === ids[0]);
      label = item ? item.name : ids[0];
    } else {
      label = ids[0];
    }
  } else if (ids.length > 1) {
    label = tFmt("market.filter.selectedN", { n: ids.length });
  }
  return `
    <button type="button" class="market-filter-chip ${ids.length ? "active" : ""}" data-filter="${filterName}">
...
async function openMarketFilterPicker(type) {
  const f = marketState.filtersDraft;
  ...
  if (type === "gift") {
    ...
    openMarketPicker({
      ...
      onToggle: (val) => {
        f.giftId = val;
        f.modelIds = []; // при смене подарка модели сбрасываются
        renderMarketListingsFromCache();
      },
    });
    return;
  ...
  if (type === "model") {
    ...
    openMarketPicker({
      ...
      mode: "multi",
      current: f.modelIds,
      onToggle: (ids) => { f.modelIds = ids; renderMarketListingsFromCache(); },
    });
    return;
  }
  ...
  if (type === "background") {
    ...
    openMarketPicker({
      ...
      items,
      mode: "multi",
      current: f.backgroundNames,
      onToggle: (names) => { f.backgroundNames = names; renderMarketListingsFromCache(); },
    });
    return;
  }
  ...
  if (type === "pattern") {
    ...
    openMarketPicker({
      ...
      items,
      mode: "multi",
      current: f.patternIds,
      onToggle: (ids) => { f.patternIds = ids; renderMarketListingsFromCache(); },
    });
    return;
  }
  ...
  if (type === "sort") {
    ...
    openMarketPicker({
      ...
      current: f.sort,
      searchable: false,
      onToggle: (val) => { f.sort = val; renderMarketListingsFromCache(); },
    });
    return;
  }
}
...
function openMarketPicker(opts) {
  ...
  const resetBtn = document.getElementById("market-picker-reset");
  if (!overlay) return;
  ...
  let selectedSet = mode === "multi" ? new Set(opts.current || []) : null;
  let singleValue = mode === "single" ? opts.current : null;
  ...
  function isActive(value) {
    ...
    if (mode === "multi") return selectedSet.has(value);
  ...
  function renderList() {
    ...
    listEl.innerHTML = rendered.map((it, i) => {
      ...
      const active = isActive(it.value) ? "selected" : "";
      const check = mode === "multi"
        ? `<span class="market-picker-check ${active ? "on" : ""}"><span class="cell-icon cell-icon-sm" data-icon="check"></span></span>`
        : "";
    ...
    listEl.querySelectorAll(".market-picker-item").forEach((b) => {
      b.addEventListener("click", () => {
        ...
        if (mode === "multi") {
          if (it.value === null) {
            selectedSet = new Set();
          } else if (selectedSet.has(it.value)) {
            selectedSet.delete(it.value);
          } else {
            selectedSet.add(it.value);
          }
          renderList();
          if (opts.onToggle) opts.onToggle([...selectedSet]);
        } else {
          singleValue = it.value;
          renderList();
          if (opts.onToggle) opts.onToggle(it.value);
        }
      });
    });
  }
  ...
  // 🔴 «Сбросить» — сбрасывает ТОЛЬКО текущий фильтр, окно остаётся открытым.
  if (resetBtn) resetBtn.onclick = () => {
    if (mode === "multi") {
      ...
      singleValue = (opts.defaultValue !== undefined) ? opts.defaultValue : null;
      renderList();
...
async function refreshMyGiftsCount() {
  ...
  // 🔴 Минус подарки, выставленные на маркет — их в профиле не видно.
  let listed = 0;
  try {
    const { count: lc } = await supabase.from("market_listings")
      .select("id", { count: "exact", head: true })
      .eq("seller_id", currentUser.id).eq("status", "active");
    listed = lc || 0;
  } catch (e) { /* silent */ }
  const visible = Math.max(0, (count || 0) - listed);
  const el = document.getElementById("profile-gifts-count");
...
function preloadPatternIcons() {
  ...
  GIFT_PATTERNS.forEach((p) => {
    if (!p.icon) return;
    // 1) SVG-маска (для плитки/hero/карточки в чате).
    buildPatternMaskUrl(p.icon).catch(() => {});
    // 2) Сырой URL — чтобы пикер фильтра показал кружок мгновенно.
    try {
      const img = new Image();
      img.decoding = "async";
      img.src = p.icon;
...
async function renderGiftDetail(ownerId, ug, opts) {
    ...
    <div class="gift-detail">
      ...
      <div class="gift-detail-actions">
        ${isOwner ? `
          ...
          <button class="dialog-btn" id="gift-market"><span class="cell-icon cell-icon-sm" data-icon="market" style="vertical-align:-3px;margin-right:6px;"></span>${escapeHtml(t("gifts.action.toMarket"))}</button>
  ...
  if (isOwner && !readOnly) {
    ...
    const marketBtn = document.getElementById("gift-market");
    if (marketBtn) marketBtn.addEventListener("click", () => marketListGiftDialog(ug.id));
